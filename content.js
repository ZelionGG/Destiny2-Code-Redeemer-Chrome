...
window.addEventListener('load', function() {
  ...
  if (isRedeemPage()) {
    ...
    chrome.runtime.sendMessage({ action: 'pageIsReady', isRedeemPage: true });
  }
});
...
chrome.runtime.onMessage.addListener(function (request, sender, sendResponse) {
  ...
  if (request.action === 'redeemCodes') {
    ...
    if (!isRedeemPage()) {
      ...
      sendResponse({ status: 'error', message: 'Not on redeem page' });
      return true;
    }
  ...
  if (request.action === 'checkRedeemPage') {
    ...
    sendResponse({ isRedeemPage: isOnRedeemPage });
    return true;
  }
