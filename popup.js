document.addEventListener('DOMContentLoaded', function () {
  ...
  function checkCurrentPage() {
    chrome.tabs.query({ active: true, currentWindow: true }, function (tabs) {
      ...
      chrome.tabs.sendMessage(currentTab.id, { action: 'checkRedeemPage' }, function (response) {
  ...
  redeemCodesButton.addEventListener('click', function () {
    ...
    chrome.tabs.query({ active: true, currentWindow: true }, function (tabs) {
      ...
      if (!currentTab.url.includes('bungie.net') || !currentTab.url.includes('Codes/Redeem')) {
        ...
        chrome.tabs.create({ url: 'https://www.bungie.net/7/en/Codes/Redeem' });
      ...
      chrome.tabs.sendMessage(currentTab.id, { action: 'ping' }, function (response) {
        ...
        chrome.tabs.sendMessage(currentTab.id, { action: 'redeemCodes', codes: cleanCodes }, function (response) {
          if (chrome.runtime.lastError) {
            console.log("Error sending codes to content script:", chrome.runtime.lastError);
  ...
  stopRedeemCodesButton.addEventListener('click', function () {
    chrome.tabs.query({ active: true, currentWindow: true }, function (tabs) {
      ...
      chrome.tabs.sendMessage(currentTab.id, { action: 'stopRedemption' }, function (response) {
        if (chrome.runtime.lastError) {
